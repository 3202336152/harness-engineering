...
- 如果 `harness/.harness/` 不存在，且当前请求涉及实现、改代码、补 spec、规划功能或接入约束，先提示“当前项目尚未初始化 harness”，并建议优先运行 `bash scripts/init-harness.sh`。
- 在 harness 初始化完成前，不进入真实功能开发或大规模代码修改；只读分析、答疑、现状梳理可以继续。
- 如果 `harness/.harness/runtime/last-audit.json` 不存在，或其中 `status` 为 `never_run`，在开始较大功能开发、重构或治理前，优先建议先运行一次 `bash scripts/audit-harness.sh` 建立基线。
...
项目级架构规范见 `harness/docs/project/项目架构.md`。

## 文档导航

- 核心信念与设计决策：`harness/docs/project/核心信念.md`
- 项目级架构：`harness/docs/project/项目架构.md`
- 项目级设计：`harness/docs/project/项目设计.md`
- 项目级接口规范：`harness/docs/project/接口规范.md`
- 项目级开发规范：`harness/docs/project/开发规范.md`
- 项目级测试策略：`harness/docs/project/测试策略.md`
- 项目级安全规范：`harness/docs/project/安全规范.md`
- 文档影响规则：`harness/.harness/doc-impact-rules.json`
- 功能级 spec：`harness/docs/features/`

## 关键约束

> **开始功能开发或进入大规模编码前，必须先确认 spec 和文档前置条件已经满足；未满足时先补文档，再写代码。**

1. 对重要功能变更，或涉及接口、数据库、安全、配置、部署的改动，必须先确认 `harness/docs/features/<feature-id>/` 下已有对应 spec，且没有 `TODO`、`TBD`、`待补充` 等占位内容。
...
3. 修改接口、数据库、安全、配置或部署相关代码前，先对照 `harness/.harness/doc-impact-rules.json`，确认需要同步更新的项目级和功能级文档。
4. 每次提交前都检查 spec、测试和相关文档是否需要同步更新；如果项目已启用 Git hook、Husky 或 CI 门禁，优先让门禁脚本阻断不完整提交。
5. 修改核心基础设施前先查看 `harness/docs/project/核心信念.md`，避免破坏长期稳定的架构决策。
6. 优先复用共享能力，避免复制粘贴式实现；对 Java 项目，优先遵循 `interfaces -> application -> domain -> infrastructure` 的分层边界。

## 自动 Audit 触发条件

- 如果 `harness/.harness/runtime/last-audit.json` 缺失，或 `last_run_at` 距当前已超过 7 天，在继续较大功能开发前优先运行 `bash scripts/audit-harness.sh`。
...
- `harness/docs/project/*.md` 和 `harness/docs/features/*/*.md` 新生成时默认会带 `doc_state: scaffold`，这表示它们只是骨架，不是已经核实过的项目真相。
- 对 Java 项目，先运行或刷新 `bash scripts/scan-java-project.sh --json`，确保 `harness/.harness/runtime/java-doc-scan.json` 是最新的。
...
- 在补全 `harness/docs/project/` 前，当前编码代理必须先读取扫描清单和关键文件，禁止依赖猜测或只看单个类就下结论。
