...
对于复杂 Java 后端项目，这份文档必须优先讲清楚当前系统的模块边界、主链路、关键表、消息链路、事务边界和排障入口，而不是只写抽象原则。
...
推荐 Java 服务优先采用以下分层，单体、多模块、微服务都可参考。对于 Spring Boot 项目，建议启动类放在根包，确保组件扫描、实体扫描和配置扫描都只覆盖当前项目范围。
